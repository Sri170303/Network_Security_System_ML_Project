...
class DataValidation:
    ...
    def detect_dataset_drift(self, base_df, curr_df, threshold = 0.05) -> bool:
        try:
            ...
            for column in base_df.columns:
                d1 = base_df[column]
                d2 = curr_df[column]
                is_sample_same = ks_2samp(d1, d2)
                if threshold <= is_sample_same.pvalue:
                    is_found = False
                else:
                    is_found = True
                    status = False
                report.update(
                    {
                        column:{
                            "pvalue":float(is_sample_same.pvalue),
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            if not status:
                error_message=f"Train DataFrame has different number of columns from expected schema.\n"
            status = self.validate_no_of_columns(test_dataframe)
            if not status:
                error_message = f"Test DataFrame has different number of columns from expected schema.\n"
            status = self.detect_dataset_drift(base_df=train_dataframe, curr_df=test_dataframe)
            dir_path = os.path.dirname(self.data_validation_config.valid_train_data_path)
            os.makedirs(dir_path, exist_ok=True)
            train_dataframe.to_csv(
                self.data_validation_config.valid_train_data_path, index=False, header=True
            )
            test_dataframe.to_csv(
                self.data_validation_config.valid_test_data_path, index=False, header=True
            )
